...
const EditProductSection = ({
  ...
  const [draftProductData, setDraftProductData] = useState<
    ProductsResponseInterface | undefined | null
  >();
  ...
  const updateProductMutation = useMutation({
    ...
    onSuccess: () => {
      openNotification(
        api,
        "Hoorayy! you've have successfully edited a product.",
        "Product Edited!"
      );
  ...
  const onSubmit: SubmitHandler<CreateProductFormValues> = (data) => {
    if (productdata && productdata.category && productdata.category.id) {
  ...
  useEffect(() => {
    if (productdata && productdata.category && productdata.category.id) {
      let tempImg = [];
      for (let item in productdata.images) {
        tempImg.push(
      ...
      setImageForm(tempImg);
      reset({
        title: productdata.title,
        description: productdata.description,
        price: productdata.price,
        categoryId: productdata.category.id ? productdata.category.id : 0,
        images: tempImg,
      });
    } else {
    }
                    ...
                    render={({ field, fieldState }) => (
                      <div className="mb-6">
                        ...
                        {/* {errors?.images?.[key] && fieldState.isTouched && (
                          <p className="text-red-500 text-sm absolute">
                            {errors?.images?.[key].message?.toString()}
                          </p>
                        )} */}
                      </div>
                    )}
